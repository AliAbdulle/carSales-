...
const sales = salesByWeek[0]
display.innerHTML += "<h1>Weekly Sales Report</h1>";

salesByWeek.forEach(sales => {
    display.innerHTML +=`<h2>${sales.sales_agent.first_name}  ${sales.sales_agent.first_name}</h2>`
    const arrayOfObjectEntries = Object.entries(sales.vehicle)
    console.log(arrayOfObjectEntries)
    // Iterate all of the values of the current sales
    for (keyValueArray of arrayOfObjectEntries) {
        console.log(keyValueArray)
        display.innerHTML += `<div>${keyValueArray[0]}: ${keyValueArray[1]}`
    }
    display.innerHTML +=`<h3> profit: $${sales.gross_profit}</h3>`
})
